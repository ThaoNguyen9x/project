...
import { Button, Descriptions, Drawer, Space, Rate, Steps } from "antd";
import {
  FORMAT_DATE_DISPLAY,
  ...
  const generateItems = () => {
    if (data?.floor) {
      ...
      return [
        ...
        {
          label: "Vị trí",
          children: data?.location?.floor ? (
            <a
              onClick={async () => {
                const res = await callGetLocation(data?.location?.id);
                if (res?.data) {
                  handleViewDetail(res?.data);
                }
              }}
